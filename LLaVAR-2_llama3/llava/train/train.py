...
from llava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, VISION_TOKEN, TEXT_TOKEN
...
from llava.model.apply_lavin import set_lavin
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        # list_data_dict=[]
        # with open(data_path, 'r') as ndjson_file:
        #     for line in ndjson_file:
        #         json_object = json.loads(line.strip())
        #         list_data_dict.append(json_object)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if self.data_args.lavin_enable:
            data_dict['input_ids'] = torch.cat([data_dict['input_ids'][:1],torch.tensor([self.tokenizer.convert_tokens_to_ids(modality_token)],dtype=torch.long), data_dict['input_ids'][1:]], 0)
            data_dict['labels'] = torch.cat([data_dict['labels'][:1], torch.tensor([IGNORE_INDEX], dtype=torch.long), data_dict['labels'][1:]], 0)
...
def train(attn_implementation=None):
    ...
    if training_args.lora_enable:
        ...
        state_dict = get_peft_state_maybe_zero_3(model.named_parameters(), 'adapter')
        non_lora_state_dict = get_peft_state_non_lavin_maybe_zero_3(model.named_parameters())
