...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    ...
    if 'llava' in model_name.lower():
        if 'phi3' in model_name.lower():
            LlavaForCausalLM=LlavaPhiForCausalLM
            if ('lora' in model_name.lower()):
                lora_cfg_pretrained = LlavaPhiConfig.from_pretrained(model_path)
        else:
            LlavaForCausalLM=LlavaLlamaForCausalLM
            if ('lora' in model_name.lower()):
        ...
        if ('lora' in model_name.lower() or 'lavin' in model_name.lower()) and model_base is not None:
            ...
            model = LlavaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=lora_cfg_pretrained, **kwargs)
            ...
            if enable_lavin:
                ...
                from llava.constants import VISION_TOKEN,TEXT_TOKEN
                ...
                model.config.vocab_size = vocab_siz
            ...
            if 'mpt' in model_name.lower():
                ...
                model = LlavaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=cfg_pretrained, **kwargs)
